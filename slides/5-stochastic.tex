...
\begin{frame}{\tcv{} The \lucie{} selection procedure}%
    \vspace{1em}
    \onslide<2->{%
        \textbf{Objective:} identify the {\color{cyan}\textbf{best $\mu$}} individuals with as {\color{magenta}\textbf{few evaluations}} as possible. \cite{lecarpentierLUCIEEvaluationSelection2022}
    }
    \vspace{1em}
    \begin{figure}
        \def\xmax{10}%
        ...
        \begin{center}
            \begin{figure}
                ...
                %uncomment if require: \path (0,300); %set diagram left start at 0, and has height of 300
                    ...
                    \draw (190,105.96) node [anchor=north west][inner sep=0.75pt]  [font=\small]  {$p( z,\ \theta _{t})$};   
                    }
                    \onslide<9->{
                    %Shape: Boxed Bezier Curve [id:dp19284355666403086] 
